...
import os
import sys
# from torcheval import Accuracy
...
def classifier(
    ...
    dataloader = DataLoader(
        datasets.FashionMNIST("Datasets/fdata", train=True, download=True, transform=transform),
    ...
    for epoch in range(epochs):
        for i, (images, labels) in enumerate(dataloader):
            ...
            # Calculate accuracy
            acc_value = 0#acc(outputs, labels)
            ...
            if (i + 1) % 100 == 0:
                print(f'Epoch [{epoch + 1}/{epochs}], Step [{i + 1}/{len(dataloader)}], \
                         Loss: {loss.item():.4f}, accuracy: {acc_value:.4f}')
...
def test_classifier():
    ...
    test_loader = DataLoader(
        datasets.FashionMNIST("Datasets/fdata", train=False, download=True, transform=transform),
